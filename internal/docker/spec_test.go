...
func TestBuildArgv_Minimal(t *testing.T) {
	...
	want := []string{
		"build",
		"-f", "/home/user/.makeslop/Dockerfile",
		"-t", "claudebox",
		"/tmp/ctx",
	}
	got := BuildArgv(o)
...
func TestBuildArgv_WithNoCache(t *testing.T) {
	o := BuildOptions{
		Image:          "claudebox",
		DockerfilePath: "/home/user/.makeslop/Dockerfile",
		ContextDir:     "/tmp/ctx",
	...
	if len(got) == 0 || got[0] != "build" {
		t.Fatalf("argv[0] = %q, want 'build'", got[0])
	}
	// --no-cache must appear immediately after "build" and before -f
	if got[1] != "--no-cache" {
		t.Errorf("argv[1] = %q, want '--no-cache'", got[1])
	}
	want := []string{
		"build", "--no-cache",
		"-f", "/home/user/.makeslop/Dockerfile",
...
func TestBuildArgv_WithMultipleBuildArgs(t *testing.T) {
	o := BuildOptions{
		Image:          "claudebox",
		DockerfilePath: "/home/user/.makeslop/Dockerfile",
		ContextDir:     "/tmp/ctx",
		BuildArgs:      []string{"GO_VERSION=1.26.3", "HTTP_PROXY=http://proxy:3128"},
	}
	want := []string{
		"build",
		"-f", "/home/user/.makeslop/Dockerfile",
...
func TestBuildArgv_ContextDirIsLast(t *testing.T) {
	o := BuildOptions{
		Image:          "myimage",
		DockerfilePath: "/some/Dockerfile",
		ContextDir:     "/the/context",
		NoCache:        true,
	...
	if last := got[len(got)-1]; last != "/the/context" {
		t.Errorf("last argv token = %q, want '/the/context'", last)
	}
}

func TestBuildArgv_NoBuildArgsNoCacheEmitsNoExtraTokens(t *testing.T) {
	o := BuildOptions{
		Image:          "claudebox",
		DockerfilePath: "/home/user/.makeslop/Dockerfile",
		ContextDir:     "/tmp/ctx",
	...
	got := BuildArgv(o)
	// Expect exactly: build -f <path> -t <image> <ctx> = 6 tokens
	const wantLen = 6
	if len(got) != wantLen {
		t.Errorf("argv len = %d, want %d; argv = %#v", len(got), wantLen, got)
	}
	for _, tok := range got {
		if tok == "--no-cache" {
			t.Errorf("unexpected --no-cache in argv: %#v", got)
		}
		if tok == "--build-arg" {
			t.Errorf("unexpected --build-arg in argv: %#v", got)
		}
	}
}

func TestBuildArgv_OrderIsDeterministic(t *testing.T) {
	o := BuildOptions{
		Image:          "claudebox",
		DockerfilePath: "/home/user/.makeslop/Dockerfile",
		ContextDir:     "/tmp/ctx",
