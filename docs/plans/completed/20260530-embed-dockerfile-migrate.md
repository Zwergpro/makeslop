...
- [x] update `README.md` (git-tracked): document `makeslop migrate` and the first-run flow (`init` seeds, `migrate` refreshes on upgrade)
- [x] move this plan to `docs/plans/completed/` (note: `docs/` is gitignored, so this is a local-only move)
- [x] skip `CLAUDE.md` — it is empty AND gitignored, so edits would not be committed
